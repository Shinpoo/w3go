...
alpha = data["alpha"]
d_max = data["d_max"] * len(data["People"])
d_min = data["d_min"]
...
score = {i:j["score"] for (i,j) in data["Destinations"].items()}
...
model.score = Param(model.D, initialize=score, doc='Destination scores')
...
model.fun_score = Var(doc="fun score")
model.d_tot = Var(doc="total distance", within=NonNegativeReals)
model.d_score = Var(doc="distance score", within=NonNegativeReals)
...
def objective_rule(model):
    return alpha * model.d_score + (1 - alpha) * model.fun_score
model.objective = Objective(rule=objective_rule, sense=maximize, doc='Min total distance')
...
model.C21 = Constraint(expr = sum(model.x[i] * model.score[i] for i in model.D) == model.fun_score, doc='Fun level')
model.C22 = Constraint(expr = model.d_tot == sum(model.d[i,j]*model.b[i,j] for i in model.N for j in model.N), doc="total distance")
model.C23 = Constraint(expr = model.d_score == 10 * (model.d_tot - d_max) / (d_min - d_max), doc="total distance")
